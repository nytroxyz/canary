...
BosstiarySystem.AddBossKillAmount = function(player, mtype)
    ...
    if (count < prowess and newCount >= prowess) then
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have unlocked prowess status from " .. bossTable.name .. ".")
    ...
    if (count < expertise and newCount >= expertise) then
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have unlocked expertise status from " .. bossTable.name .. ".")
        if (bossTable.type == BosstiarySystem.enums.races.archfoe) then
    ...
    if (count < mastery and newCount >= mastery) then
        player:sendTextMessage(MESSAGE_EVENT_ADVANCE, "You have unlocked mastery status from " .. bossTable.name .. ".")
        if (bossTable.type == BosstiarySystem.enums.races.archfoe) then
